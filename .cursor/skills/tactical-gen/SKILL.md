...
- Selection:
  - Choose `angerHigh` and `stressHigh` so that conditional branches are likely to trigger on a first playthrough, but require extra buildup or visits on subsequent runs.
  - Prefer thresholds that are reachable with default delta bands (2..12) within 2–4 regular passages before a conditional branch.
